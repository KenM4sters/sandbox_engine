...
void main() {
    // Ambient light
    vec3 ambient_shading = light_cube.ambient * texture(material.diffuse_forest, tex_coord).rgb;
    // Diffuse light_cube
    ...
    vec3 diffuse_shading = light_cube.diffuse * diffuse_angle * texture(material.diffuse_forest, tex_coord).rgb;
    // Specular light_cube
