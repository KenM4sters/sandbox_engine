...
void main() {
    // Ambient light
    vec3 ambient_shading = light.ambient * texture(material.diffuse, tex_coord).rgb;
    // Diffuse light
    vec3 normal_dir = normalize(normal);
    vec3 light_dir = normalize(light.position - frag_pos);
    float diffuse_angle = max(dot(normal_dir, light_dir), 0.0);
    vec3 diffuse_shading = light.diffuse * diffuse_angle * texture(material.diffuse, tex_coord).rgb;
    // Specular light
