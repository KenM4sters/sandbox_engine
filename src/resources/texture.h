 ...
 * or in our case "BasicMesh", the texture is intended for.
 *  
 ...
 * BasicMesh::Geometry::AddBufferAttribute(), and accessed in the shader prgrams themselves to be used.
*/
class Texture2D
{
public:
    ...
    // texture must be bound at the time of drawing the "BasicMesh" that it belongs to.
    void Bind() const;
    void Bind(unsigned int count) const;
    void Unbind() const;
};
