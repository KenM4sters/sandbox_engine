...
void SObjects::Init() {
    ...
    BasicMesh* cube_mesh = new BasicMesh(new BufferGeometry(vertices, SANDBOX_CUBE_VERTICES_COUNT), cube_shader, tex_vec);
    children_["cube"] = cube_mesh;

    // Floor
    Shader* floor_shader = shaders_->GetResource("floor");
    Texture2D* sand_tex = textures_->GetResource("sand");
    BasicMesh* floor_mesh = new BasicMesh(new BufferGeometry(vertices, SANDBOX_CUBE_VERTICES_COUNT), floor_shader, sand_tex);
